...
bool isReceiver = false; // integers 0-5 = TX node; character 'R' or integer 82 = RX node
...
// payload's nodeID
void setMode(bool receiver); // declare a prototype; definition is found after the loop()
...
void setup()
{
    ...
    radio.setPayloadSize(sizeof(MessageRec)); // 2x int datatype occupy 8 bytes
...
void loop()
{
    ...
    if (!isReceiver)
    {
        ...
        if (report)
        {
            ...
            Serial.print(payload.id); // print payloadID
            Serial.print(F(" as node "));
            Serial.print(payload.from); // print nodeID
        ...
        payload.id++; // increment payload number
    ...
    else if (isReceiver)
    {
        ...
        {                                           // is there a payload? get the pipe number that received it
            ...
            Serial.print(payload.from); // print the payload's origin
            Serial.print(F(". PayloadID: "));
            Serial.println(payload.id); // print the payload's number
        }
        else
        {
...
void setMode(bool bReceiver)
{
    ...
    else
    {
        ...
        payload.from = TERMINAL_ID;
        payload.id = 0;

        // set the TX address of the RX node for use on the TX pipe (pipe 0)
        radio.stopListening(address[isReceiver]); // put radio in TX mode

        // According to the datasheet, the auto-retry features's delay value should
        // be "skewed" to allow the RX node to receive 1 transmission at a time.
        // So, use varying delay between retry attempts and 15 (at most) retry attempts
        radio.setRetries(((isReceiver * 3) % 12) + 3, 15); // maximum value is 15 for both args
    }
} // setRole
